...
当前仓库版本为个人定制版 `v9.1.2-personal` (个人弄着玩的) 🍀：

- **动画优化**：修复评论区面板在退出/关闭时瞬间消失、不播放淡出与下滑动画的问题。
- **遮罩修复**：修复展开楼中楼评论详情时，视频播放器区域被灰色半透明蒙层错误遮挡的问题。
- **图标定制**：更换应用默认启动图标为全新粉色 “bi” 萌系图标（包含 Legacy/Round 及自适应 Adaptive Icon 支持）。
