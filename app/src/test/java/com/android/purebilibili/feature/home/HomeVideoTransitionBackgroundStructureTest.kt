...
class HomeVideoTransitionBackgroundStructureTest {
    ...
    fun homeRootOwnsVideoTransitionBackgroundBlurAndScrim() {
        ...
        assertTrue(driverBlock.contains("durationMillis = HOME_VIDEO_TRANSITION_BACKGROUND_RETURN_DURATION_MS"))
        assertTrue(driverBlock.contains("targetValue = 0f"))
        assertTrue(!driverBlock.contains("homeVideoTransitionBackgroundProgress.snapTo(1f)"))
        assertTrue(!driverBlock.contains("HOME_VIDEO_TRANSITION_BACKGROUND_RETURN_SETTLE_PROGRESS"))
